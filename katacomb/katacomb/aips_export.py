...
def _update_target_metadata(target_metadata, image, target, tn, katds, filebase):
    ...
    radec = target.radec()
    target_metadata.setdefault('RightAscension', []).append(radec.ra.to_string(unit='h', sep=':'))
    target_metadata.setdefault('Declination', []).append(radec.dec.to_string(sep=':'))
    target_metadata.setdefault('DecRa', []).append(','.join(map(str, [radec.dec.deg, radec.ra.deg])))
        ...
        try:
            with uv_factory(aips_path=uv_file, mode='r') as uvf:
                ...
                if sntab.keywords["NO_POL"] < 2:
                    raise ValueError("Only dual-pol self-calibration solutions are supported")
